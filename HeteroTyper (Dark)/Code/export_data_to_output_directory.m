...
function data = export_data_to_output_directory(data) 
    ...
    output_dir = 'D:\Gizem\HeteroTyper\Test_Dark\Test_output\';
    if ~exist(output_dir, 'dir')
        mkdir(output_dir);
    end
    
    % Initialize one master table for all groups
    allGroupsTable_Dark     = table();
    allGroupsTable_Dark_2   = table();
    
    % Loop over all sample groups (MLN, Spleen, Liver)
    for g = 1:length(labels_type)
        ...
        allGroupsTable_Dark      = [allGroupsTable_Dark; groupTable];
        allGroupsTable_Dark_2    = [allGroupsTable_Dark_2; groupTable_2];
    end
    
    %% -- Export as one file --
    filename = fullfile(output_dir, '\Morphology_AllParameters_Dark.xlsx');
    writetable(allGroupsTable_Dark, filename);
    fprintf('✅ Exported combined morphology data for all groups to:\n   %s\nTotal colonies: %d\n', filename, height(allGroupsTable_Dark));
    
    filename_2 = fullfile(output_dir, '\Morphology_AllParameters_2_Dark.xlsx');
    writetable(allGroupsTable_Dark_2, filename_2);
    fprintf('✅ Exported combined morphology data for all groups to:\n   %s\nTotal colonies: %d\n', filename_2, height(allGroupsTable_Dark_2));

    
    %% -- Save to MATLAB workspace --
    assignin('base', 'allGroupsTable_Dark', allGroupsTable_Dark);
    mat_file1 = fullfile(output_dir, 'allGroupsTable_Dark.mat');
    save(mat_file1, 'allGroupsTable_Dark')

    assignin('base', 'allGroupsTable_2_Dark', allGroupsTable_Dark_2);
    mat_file2 = fullfile(output_dir, 'allGroupsTable_Dark_2.mat');
    save(mat_file2, 'allGroupsTable_Dark_2')
end
