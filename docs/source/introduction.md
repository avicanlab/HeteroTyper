...
This handbook is the user reference for the HeteroTyper computational pipeline, the
analysis software that complements the HeteroTyper imaging system. The HeteroTyper
system captures time-lapse images of bacterial colonies growing on agar plates and
records how each colony develops over time. The primary aim of the pipeline is to
quantify lag time at the single-colony level, the time each individual colony takes
before it adjusts themselves to the new environment and initiate their growth.
