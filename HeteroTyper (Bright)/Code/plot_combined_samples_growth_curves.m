...
function data = plot_combined_samples_growth_curves(data) 
    ...
    for i = 1:length(ix) 
        ...
        for j = 1:length(ix_tmp)  
            ...
            % Build zero index only where size is available
            has_size = ~isnan(col_size) & col_size ~= 0; % treat 0 or NaN as "no size data"
            zero_idx = (has_size & ((col_size < size_threshold) & (eccentricity >= ecc_threshold)));   
            
            % Apply filter
            col_flag(zero_idx) = 0;
            % Save back
            data.processed{ix_tmp(j)}.colonies.flag_colony_ok = col_flag;
            


            % Keep only valid entries (nonzero, finite, flagged colonies)
            valid_idx = (col_flag == 1) & (col_size > size_threshold) & (eccentricity < ecc_threshold);
