...
function data = plot_combined_samples(data) 
    ...
    xval_area       = [0:50:4100];
    xval_int        = [0:0.01e5:1.71e5];
    xval_mean_int   = [0:0.05:50.05];
    xval_intSize    = [0:1:101];
    xval_peri       = [0:5:245];
    xval_sol        = [0:0.01:1.01];
    xval_circ       = [0:0.01:1.01];
    xval_ecc        = [0:0.01:1.01];
    ...
    for i = 1:length(ix) 
        ...
        for j = 1:length(ix_tmp)  
            ...
            xval_plot1 = 0.25:0.5:51.75; 
            bar(xval_plot1,combined_counts./sum(combined_counts),'FaceColor',colors(i,:),'EdgeColor','none','FaceAlpha',1); 
            hold on; 
            plot(xval_plot1, combined_counts./sum(combined_counts),'Color', 'k', 'LineWidth', 0.3);  % 'k' for black line 
            axis([incTime inf 0 max(combined_counts./sum(combined_counts))]); 
            xticks(incTime:10:max_lag); 
            line([median(combined_lag_time) median(combined_lag_time)],[0 max(combined_counts./sum(combined_counts))],'Color','r','LineWidth',1.5);
            set(gca, 'FontSize', 11); 

            % Final Colony Size
            subplot(length(ix), 10, 10 * (i - 1) + 2),... 
            xval_plot2 = 0:50:4050; 
            ...
            line([median(combined_size) median(combined_size)],[0 max(combined_counts2./sum(combined_counts2))],'Color','r','LineWidth',1.5); 
            set(gca, 'FontSize', 11); 

            % Area
            subplot(length(ix), 10, 10 * (i - 1) + 3),... 
            xval_plot3 = 0:50:4050; 
